...
let handler = async (m, { conn, text, usedPrefix, isOwner, participants }) => {
  function no(number){
  ...
  m.reply("DONE")
  //conn.reply("120363047029251042@g.us",`*BANNED NOTIFICATION*\n\nwa.me/${m.sender.split('@')[0]} _Telah membanned_ @${hl[0].split('@')[0]}`,m,{ contextInfo: { mentionedJid: [hl[0]] } })
  conn.sendMessage(m.chat, { text: `*BANNED NOTIFICATION*\n\n_Telah Terbanned_ @${hl[0].split('@')[0]}`, mentions: participants.map(a => a.id) }, { contextInfo: { mentionedJid: [hl[0]] } })
}
handler.help = ['oban <@tag/number>']
handler.tags = ['owner']
